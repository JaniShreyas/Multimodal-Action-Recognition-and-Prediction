...
class EpicKitchens100Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        video_id, start_frame, stop_frame, narration, verb_class = row[
            ["video_id", "start_frame", "stop_frame", "narration", "verb_class"]
        ]
        participant_id = video_id.split("_")[0]
        frames_dir = os.path.join(self.root_dir, participant_id, "rgb_frames")
        frames = self.get_frames_for_segment(
            frames_dir, video_id, start_frame, stop_frame
        )
        sample = {
            "video_id": video_id,
            "frames": frames,
